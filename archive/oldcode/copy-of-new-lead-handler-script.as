...
// Geocoding - wrapped in try-catch to prevent errors
   let geoData = null;
   try {
     geoData = getLatLng(location);
   } catch (err) {
     console.error(`⚠️ Geocoding failed for location "${location}": ${err.message}`);
     geoData = null; // Safe fallback
   }
