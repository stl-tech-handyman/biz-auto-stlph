#!/bin/bash
# Test Go API in Development Environment
...
print_status "Testing DEV environment: $SERVICE_URL"

# Get API key from Secret Manager
API_KEY=$(gcloud secrets versions access latest --secret="svc-api-key-dev" --project="$PROJECT_ID" 2>/dev/null)
...
# Test deposit calculation (requires auth)
print_status "Testing deposit calculation..."
DEPOSIT_RESPONSE=$(curl -s -w "\n%{http_code}" \
    -H "X-Api-Key: $API_KEY" \
    "$SERVICE_URL/api/stripe/deposit/calculate?estimate=1000")
HTTP_CODE=$(echo "$DEPOSIT_RESPONSE" | tail -n1)
BODY=$(echo "$DEPOSIT_RESPONSE" | sed '$d')

if [ "$HTTP_CODE" = "200" ]; then
    print_success "Deposit calculation passed"
    echo "$BODY" | jq '.' 2>/dev/null || echo "$BODY"
else
    print_error "Deposit calculation failed: HTTP $HTTP_CODE"
    echo "$BODY"
    exit 1
fi
